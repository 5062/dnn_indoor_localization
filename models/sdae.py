...
def sdae(input_data=None,
    ...
    if cache:
        if model_fname is None:
            model_fname = './saved/sdae/' + 'ujiindoorloc' + '/H' \
    ...
    for i in range(n_hl):
        encoded_input.append(Input(shape=(all_layers[i],), name='sdae_encoded_input' + str(i)))
        encoded.append(Dense(all_layers[i + 1], activation='sigmoid')(encoded_input[i]))
        decoded.append(Dense(all_layers[i], activation='sigmoid')(encoded[i]))
        autoencoder.append(Model(inputs=encoded_input[i], outputs=decoded[i]))
        # encoder.append(Model(inputs=encoded_input[i], outputs=encoded_bn[i]))
    ...
    if cache:
        pathlib.Path(os.path.dirname(model_fname)).mkdir(parents=True, exist_ok=True)
        model.save(model_fname)  # save for later use

        with open(os.path.splitext(model_fname)[0] + '.org', 'w') as output_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_cache",
        help="disable loading a trained model from/saving it to a cache",
        action='store_true')
    parser.add_argument(
        "--validation_split",
        help="fraction of training data to be used as validation data: default is 0.2",
        default=0.2,
        type=float)
    parser.add_argument(
        "-H",
        "--hidden_layers",
        help="comma-separated numbers of units in SDAE hidden layers; default is '128,128,128'",
        default='128,128,128',
        type=str)
    parser.add_argument(
        "-F",
        "--frac",
        help="fraction of input data to load for training and validation; default is 1.0",
        default=1.0,
        type=float)
    parser.add_argument(
        "-P",
        "--preprocessor",
        help="preprocessor to scale/normalize input data before training and validation; default is 'standard_scaler'",
